...
export const createBooking = errorHandler(async (bookingInfo, userId) => {
...
export const getBookingById = errorHandler(async (bookingId, userId) => {
    const booking = await Booking.findById(bookingId);
    if (!booking) {
        throw new NotFoundError(`Booking id not found`);
    }
    if (booking.user.toString() !== userId)
        throw new Error("unAuthorizes to book");
    return booking;
});
export const updateBookingPayment = errorHandler(async (bookingId, bookingInput, user) => {
...
export const getBookedDays = errorHandler(async (roomId) => {
...
export const getBookingByUserId = errorHandler(async (userId) => {
...
export const getBookingForInvoice = errorHandler(async (bookingId) => {
...
export const getDashBoardMetaData = errorHandler(async (startDate, endDate) => {
    ...
    const { salesData: salesDataResult = [], pendingCashData: pendingCashDataResult = [], paidCashData: paidCashDataResult = [] } = saleDataInfo[0] || {};
    let totalSales = 0;
    let totalBooking = 0;
    const saleMap = new Map();
    salesDataResult.forEach((data) => {
...
export const getAllBookings = errorHandler(async (user, page, perPage) => {
    const isAdmin = user.role?.includes("admin");
    if (!isAdmin)
        throw new Error("UnAuthorized to see these");
